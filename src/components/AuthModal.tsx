...
export function AuthModal({
  ...
  // Sign in state — default to Rental Staff / Admin
  const [email, setEmail] = useState(
    currentUser.role === "rental_staff" ? currentUser.email : "admin@rentsense.com",
  );
  ...
  const [regCompany, setRegCompany] = useState("RentSense Operations HQ");
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    switchUserRole("rental_staff");
    setStatusMsg(`Authenticated as Vikram Singh (Rental Staff / Admin)`);
    setTimeout(() => {
      onClose();
      setStatusMsg("");
    }, 500);
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    const created = registerUser({
      ...
      phone: regPhone.trim(),
      role: "rental_staff",
      companyName: regCompany.trim() || "RentSense Operations HQ",
    });

    setStatusMsg(`Account Created! Welcome ${created.name} (Rental Staff / Operations Lead).`);
    setTimeout(() => {
      onClose();
      setStatusMsg("");
    }, 600);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-foreground/25 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-md overflow-hidden rounded-[28px] border border-border/80 bg-white p-7 shadow-float animate-scale-in max-h-[92vh] overflow-y-auto">
        ...
        <button
          onClick={onClose}
          className="absolute right-5 top-5 flex h-8 w-8 items-center justify-center rounded-full text-muted-foreground hover:bg-muted cursor-pointer"
        >
          <X size={16} />
        </button>
        ...
        <div className="text-center">
          ...
          <p className="text-[12.5px] text-muted-foreground mt-0.5 font-medium">
            Rental Operations &amp; Fleet Admin Portal
          </p>
        </div>
        ...
        {authMode === "signin" ? (
          /* SIGN IN FORM (Rental Staff / Admin) */
          <form onSubmit={handleLogin} className="mt-5 space-y-3.5 text-[13px] animate-fade-in">
            <div className="rounded-2xl border border-border/60 bg-muted/20 p-3 text-[12px] flex items-center gap-2">
              <ShieldCheck size={16} className="text-ok shrink-0" />
              <div>
                <span className="font-bold text-foreground block">Rental Staff / Admin Authentication</span>
                <span className="text-[11px] text-muted-foreground">Authorizes full fleet operations and dispatch control</span>
              </div>
            </div>
            ...
            <button
              type="submit"
              className="w-full flex items-center justify-center gap-2 rounded-full bg-accent px-5 py-3 text-[13px] font-bold text-accent-foreground shadow-xs hover:opacity-90 active:scale-[0.98] transition-all mt-2 cursor-pointer"
            >
          ...
          /* REGISTER NEW ADMIN ACCOUNT FORM */
          <form onSubmit={handleRegister} className="mt-5 space-y-3 text-[13px] animate-fade-in">
            <div className="rounded-2xl border border-border/60 bg-muted/20 p-3 text-[12px] flex items-center gap-2">
              <ShieldCheck size={16} className="text-ok shrink-0" />
              <div>
                <span className="font-bold text-foreground block">Operations Lead Registration</span>
                <span className="text-[11px] text-muted-foreground">Creates verified rental management credentials</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. RentSense Central HQ"
                  className="w-full rounded-2xl border border-border/80 bg-muted/30 px-4 py-2 text-foreground font-medium outline-none focus:border-foreground focus:bg-white"
                />
              </div>
            </div>
