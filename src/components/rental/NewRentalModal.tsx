...
export function NewRentalModal({
  ...
  const isStaff = currentUser.role === "rental_staff" || currentUser.role === "supervisor_admin";
  ...
  // The rental team can override the list rate/deposit per contract — two
  // customers renting the same asset can be quoted different amounts.
  // Defaults come from the equipment's list rate, but nothing here is a
  // fixed ₹50,000: change the asset and these numbers change with it.
  ...
  const handleBookRental = () => {
    if (!agreementChecked || monthlyRate <= 0 || securityDeposit < 0) return;
    setSubmitting(true);
        ...
        {successContract ? (
          ...
          <div>
            ...
            <div className="mt-5 rounded-2xl border border-border/70 bg-card p-4 space-y-2 text-[12.5px]">
              <div className="flex items-center justify-between">
                ...
                {isStaff && (
                  <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-bold text-muted-foreground">
                    Editable by Rental Staff
                  </span>
                )}
              </div>

              <div className="flex items-center justify-between py-1 border-b border-border/40">
                <span className="text-muted-foreground">Monthly Rental Amount:</span>
                {isStaff ? (
                  <div className="flex items-center gap-1">
                    ...
                    <input
                      ...
                      className="w-28 rounded-lg border border-border bg-white px-2 py-1 text-right font-bold text-foreground outline-none tabular-nums"
                    />
                  </div>
                ) : (
              ...
              <div className="flex items-center justify-between py-1 border-b border-border/40">
                <div>
                  <span className="text-foreground font-semibold">Security Deposit:</span>
                  <span className="ml-2 rounded-full bg-ok/15 px-2 py-0.2 text-[10px] font-bold text-ok">
                    REFUNDABLE
                  </span>
                  <p className="text-[10.5px] text-muted-foreground">
                    Held in escrow, kept separate from rental revenue. Refunded minus any approved deduction post-inspection.
                  </p>
                </div>
                {isStaff ? (
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <input
                      ...
                      className="w-28 rounded-lg border border-border bg-white px-2 py-1 text-right font-bold text-ok outline-none tabular-nums"
                    />
                  </div>
                ) : (
